...
用法（RK3588 板端，.so 已部署到 vision_algorithm/detection/）：
  python test/ab_cpp_engine.py                  # 精度 + 耗时对比（both）
  python test/ab_cpp_engine.py --frames 60      # 采集帧数
  python test/ab_cpp_engine.py --mode cpp       # 单测 cpp 引擎耗时
  python test/ab_cpp_engine.py --mode lite      # 单测 lite 引擎耗时
