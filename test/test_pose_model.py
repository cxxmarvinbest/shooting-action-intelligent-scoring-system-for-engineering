...
用法（RK3588 板上，项目根目录）：
    python3 test/test_pose_model.py <图片路径> [--model weights/yolov8n-pose.rknn] [--out 结果图]
    python3 test/test_pose_model.py <图片路径> --crop x1,y1,x2,y2   # 先抠 ROI 再测（复现检测通路）
    python3 test/test_pose_model.py <图片路径> --conf 0.1 --kpt 0.3  # 临时调阈值排查
"""
...
def main():
    ...
    kpt = args.kpt if args.kpt is not None else 0.5
