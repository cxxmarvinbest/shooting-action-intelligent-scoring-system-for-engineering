...
//         printf("MPP HEVC 解码器初始化成功\n");
        ...
        // 发送EOS指令清空解码缓存
        if(mpp_ctx && mpp_api){
            mpp_api->control(mpp_ctx, MPP_STOP, nullptr);
            // 读取并释放所有残留帧
    ...
    void stop() {
        ...
        if (decode_thread_.joinable()) {
            ...
            if (decode_thread_.joinable()) {
                if (!running_) {
                    decode_thread_.join();   // 立即返回
                    printf("线程已退出，成功 join\n");
                } else {
                    fprintf(stderr, "警告：解码线程 5 秒后仍未退出，强制 detach\n");
