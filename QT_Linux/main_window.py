...
依赖：PyQt5 / http_client / render_widget / fonts
"""
...
class FramePoller(QThread):
    ...
    def _bgr_to_qimage(raw, w, h):
        ...
        if not raw or w <= 0 or h <= 0:
            return None
        try:
            return QImage(raw, w, h, w * 3, QImage.Format_BGR888).copy()
        except Exception:
            return None
...
class MainWindow(QMainWindow):
    def __init__(self, host="127.0.0.1", port=8899):
        super().__init__()
        self.setWindowTitle("投篮动作智能评分 —— Qt 客户端 (Linux)")
    ...
    def _build_result_panel(self):
        ...
        self.kp_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.kp_table.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.kp_table.setSelectionMode(QAbstractItemView.NoSelection)
